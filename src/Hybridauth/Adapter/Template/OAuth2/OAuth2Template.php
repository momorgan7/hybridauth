...
class OAuth2Template extends AbstractAdapter implements AdapterInterface
{
	...
	*/
	function signedRequest( $uri, $method = Request::GET, $parameters = array() )
	{
		...
		switch( $method ){
			case Request::GET:
				$cache = $this->getCacheRequest($uri,array($parameters['access_token']));
				if(!is_null($cache)) {
					$this->httpClient->setResponse($cache);
				} else {
					$this->httpClient->get    ( $uri, $parameters );
					$this->setCacheResponse($uri, array($parameters['access_token']), $this->httpClient->getResponse());
				}
				break;
